...
export default function Reports() {
  ...
  const barData = (trends || []).map((m) => {
    const isPurchase = m.entries?.type === 'PURCHASE';
    const isSale = m.entries?.type === 'SALE';
    const purchases = isPurchase ? m.entries?.totalAmount : 0;
    const sale = isSale ? m.entries?.totalAmount : 0;
